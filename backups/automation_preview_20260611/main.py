...
async def start_scraper_from_phase1(
    ...
    for existing in list_scrape_jobs(1000):
        if existing.source_phase1_job_id == job.id:
            ...
            append_log(job, f"Updated existing Title Scraper job {existing.id} with {len(titles)} SAFE title(s).")
            save_job(job)
            background_tasks.add_task(run_bulk_scrape, existing.id, None)
            return RedirectResponse(url=f"/scraper/jobs/{existing.id}", status_code=303)
    ...
    append_log(job, f"Sent {len(titles)} SAFE title(s) to Title Scraper job {scrape_id}.")
    save_job(job)
    background_tasks.add_task(run_bulk_scrape, scrape_id, None)
...
""" + "".join(worksheet(name, sheet_rows(rows)) for name, rows in groups) + "</Workbook>"
    ...
    if kind in {"ready", "partial"}:
        refresh_counts(job)
        if kind == "ready" and job.export_blockers:
            raise HTTPException(status_code=409, detail={"message": "Final export is not ready.", "blockers": job.export_blockers})
...
async def download_scraper_job(scrape_id: str, kind: str = "master", user: str = Depends(authenticate)):
    ...
    if kind == "one-match":
        ...
        writer.writerow(["Input Title", "Result Title", "Result Views", "Channel ID", "Subs", "Status Tagging"])
        for row in job.results:
            writer.writerow([row.input_title, row.result_title, row.views, row.channel_id, row.subs, row.status_tagging])
    buf.seek(0)
